...
export function pglitePool(db) {
  ...
  async function rawQuery(sql,params) { if(params===undefined&&sql.split(';').filter(part=>part.trim()).length>1){await db.exec(sql);return{rows:[],rowCount:0};}const result=await db.query(sql,params?.map(value=>pgUtils.prepareValue(value)));return{...result,rowCount:result.rows.length||result.affectedRows||0}; }
