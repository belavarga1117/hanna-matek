...
function mountCognitive(ctx,gameId){
  ...
  const setPaused=(value,reason='manual')=>{if(value===clock.paused){if(value&&reason==='manual')pauseSource='manual';return;}if(value){clock.pause();pauseSource=reason;recorder.quality('pause',reason);pause.textContent='Folytatom';content.inert=true;}else{if(reason==='background'&&pauseSource!=='background')return;clock.resume();pauseSource=null;recorder.quality('resume',reason);pause.textContent='Szünet';content.inert=false;}};
  ...
  function reset(){runId++;cancelAnswer?.();clock.cancel();clock=createPausableClock();pauseSource=null;recorder.reset();spanHits.clear();stoppedDirections.clear();content.inert=false;pause.textContent='Szünet';void run(runId);}
