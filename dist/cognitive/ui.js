...
import {createBrainExplorer} from './brain-explorer.js';
...
export const COGNITIVE_TAXONOMY = Object.freeze([
  ...
  {id:'digit-span',title:'Hallott számsor',short:'Hanglánc',icon:'◌',family:'Verbális munkamemória',measures:'Hallott számjegyek sorrendjének előre és visszafelé felidézése.',limit:'A rögzített magyar hang és a lejátszóeszköz befolyásolhatja az eredményt.',network:['front','temporal','parietal']},
...
export function renderCognitiveHub({h,teacher=false}){
  ...
  let selected='spatial-span';const brain=createBrainExplorer({h,initialTask:selected,onTaskChange:updateSelection});
  function updateSelection(id){selected=id;cards.forEach((card,index)=>{const current=COGNITIVE_TAXONOMY[index].id===id;card.classList.toggle('selected',current);card.querySelector('button').setAttribute('aria-pressed',String(current));});}
  const cards=COGNITIVE_TAXONOMY.map(meta=>h('article',{className:`taxonomy-card ${meta.id===selected?'selected':''}`},h('button',{type:'button','aria-pressed':String(meta.id===selected),onClick:()=>{updateSelection(meta.id);brain.setTask(meta.id);}},h('span',{className:'taxonomy-icon','aria-hidden':'true'},meta.icon),h('span',{},h('strong',{},meta.title),h('small',{},meta.family))),h('p',{},meta.measures),h('p',{className:'taxonomy-limit'},meta.limit),h('a',{className:'primary-button',href:taskLink(meta.id,'assessment')},meta.id==='nback'?'Rögzített N-back':'Próba indítása'),h('a',{className:'text-link',href:taskLink(meta.id,'practice')},'Gyakorlás')));
...
function mountCognitive(ctx,gameId){
  ...
  const setPaused=(value,reason='manual')=>{if(value===clock.paused){if(value&&reason==='manual')pauseSource='manual';return;}if(value){clock.pause();audio?.stop();pauseSource=reason;recorder.quality('pause',reason);pause.textContent='Folytatom';content.inert=true;}else{if(reason==='background'&&pauseSource!=='background')return;clock.resume();pauseSource=null;recorder.quality('resume',reason);pause.textContent='Szünet';content.inert=false;}};
  ...
  function reset(){runId++;audio?.stop();cancelAnswer?.();clock.cancel();clock=createPausableClock();pauseSource=null;recorder.reset();spanHits.clear();stoppedDirections.clear();content.inert=false;pause.textContent='Szünet';void run(runId);}
  ...
  async function showDigit(trial,id){show('Hallott számsor',`${trial.stimulus.direction==='forward'?'Előre':'Visszafelé'} · ${trial.stimulus.length} számjegy`,h('div',{className:'audio-orb','aria-hidden':'true'},'◌'));const play=h('button',{className:'primary-button'},'Számsor lejátszása');content.append(play);await new Promise(resolve=>{play.addEventListener('click',async()=>{if(play.disabled)return;play.disabled=true;try{audio ||= await createDigitAudio({audioSetVersion:trial.stimulus.audioSetVersion});if(disposed||id!==runId)return;await audio.playDigits(trial.stimulus.audioTokens.map(token=>Number(token.split('-')[1])));if(disposed||id!==runId)return;recorder.audio(trial,'played');resolve();}catch(error){if(disposed||id!==runId)return;recorder.audio(trial,'error');play.disabled=false;content.append(h('p',{className:'input-error',role:'alert'},error.message));}});cancelAnswer=resolve;});if(disposed||id!==runId)return false;let entered=[];const digits=h('div',{className:'digit-pad'},Array.from({length:10},(_,digit)=>h('button',{className:'digit-key',onClick:()=>{if(entered.length<trial.stimulus.length){entered.push(digit);answer.textContent=entered.join(' ');}}},String(digit))));const answer=h('div',{className:'digit-answer','aria-live':'polite'},'–');show('Írd be a hallott sort',trial.stimulus.direction==='forward'?'Ugyanabban a sorrendben':'Visszafelé',answer,digits,h('button',{className:'text-link',onClick:()=>{entered.pop();answer.textContent=entered.join(' ')||'–';}},'Utolsó törlése'));const reply=await ask(trial,content,()=>[...entered],()=>entered.length===trial.stimulus.length);if(typeof reply.value==='symbol')return false;recorder.response(trial,reply.value,reply.rt);rememberSpanOutcome(trial,reply.value,trial.stimulus.audioTokens.map(token=>Number(token.split('-')[1])));return true;}
  ...
  async function delayedIntermission(){let availableAt=null;try{availableAt=await ctx.prepareDelayedRecall?.(recorder.raw());}catch(error){show('A késleltetés most nem indítható','A szerver nem tudta rögzíteni az azonnali felidézés végét.',h('p',{className:'input-error',role:'alert'},error?.message||'Ellenőrizd a kapcsolatot, majd indítsd újra a feladatot.'));return false;}let left=Math.max(0,Math.ceil((availableAt?Date.parse(availableAt)-Date.now():settings.delayedMinimumMs)/1000));const time=h('strong',{className:'intermission-time'},`${left} mp`);show('Egy hasznos közjáték','A szerver ettől a ponttól méri a késleltetést; a későbbi felidézés csak utána nyílik meg.',h('div',{className:'intermission-card'},h('span',{'aria-hidden':'true'},'◌ · ◇ · ◌'),h('p',{},'Nézz körül, lélegezz nyugodtan, vagy számolj vissza húsztól. A képek helyét ne jegyezd fel.'),time));while(left>0){if(!await wait(1000))return false;left=Math.max(0,Math.ceil((availableAt?Date.parse(availableAt)-Date.now():left*1000-1000)/1000));time.textContent=`${left} mp`;}return true;}
  ...
  async function showRecognition(trial){if(trial.kind==='recognition-study'){show('Nézd meg a képeket','Hamarosan új és korábban látott képek keverednek.',h('div',{className:'picture-study'},trial.stimulus.items.map(id=>h('span',{},pictureFor(id)))));return wait(trial.stimulus.studyMs);}show('Láttad ezt a képet?','A tanulási részben szerepelt?',h('div',{className:'recognition-picture'},pictureFor(trial.stimulus.itemId)));const choices=h('div',{className:'binary-choice'});content.append(choices);const shown=performance.now();const reply=await new Promise(resolve=>{let used=false;[['Igen, láttam','old'],['Nem, új','new']].forEach(([label,value])=>choices.append(h('button',{className:'primary-button',onClick:()=>{if(used)return;used=true;[...choices.children].forEach(b=>b.disabled=true);resolve({value,rt:Math.max(1,performance.now()-shown)});}},label)));cancelAnswer=()=>resolve({value:Symbol.for('cancel')});});if(typeof reply.value==='symbol')return false;recorder.response(trial,reply.value,reply.rt);return true;}
  async function showAttention(trial){let tapped=false;const symbol={kor:'●',haromszog:'▲',negyzet:'■',csillag:'★'}[trial.stimulus.symbol];const shown=performance.now();const tap=h('button',{className:'attention-target','aria-label':symbol==='★'?'Csillag: ne koppints':'Jelzés',onClick:()=>{if(tapped||symbol==='★'&&tap.disabled)return;tapped=true;tap.disabled=true;recorder.response(trial,true,Math.max(1,performance.now()-shown));}},symbol);show('Jelezz — a csillagnál várj',`${trial.trialIndex+1} / ${plan.trials.length}`,tap);await wait(trial.stimulus.responseWindowMs);tap.disabled=true;return wait(Math.max(0,settings.intertrialMs-trial.stimulus.responseWindowMs));}
