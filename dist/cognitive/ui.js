...
export function renderCognitiveHub({h}){
  ...
  let selected='spatial-span';const brain=createBrainExplorer({h,initialTask:selected});
  const cards=COGNITIVE_TAXONOMY.map(meta=>h('article',{className:`taxonomy-card ${meta.id===selected?'selected':''}`},h('button',{type:'button','aria-pressed':String(meta.id===selected),onClick:event=>{selected=meta.id;cards.forEach(card=>{card.classList.remove('selected');card.querySelector('button').setAttribute('aria-pressed','false');});event.currentTarget.parentElement.classList.add('selected');event.currentTarget.setAttribute('aria-pressed','true');brain.setTask(selected);}},h('span',{className:'taxonomy-icon','aria-hidden':'true'},meta.icon),h('span',{},h('strong',{},meta.title),h('small',{},meta.family))),h('p',{},meta.measures),h('p',{className:'taxonomy-limit'},meta.limit),h('a',{className:'primary-button',href:taskLink(meta.id,'assessment')},meta.id==='nback'?'Rögzített N-back':'Próba indítása'),h('a',{className:'text-link',href:taskLink(meta.id,'practice')},'Gyakorlás')));
