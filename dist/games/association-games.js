...
function mountShopping(ctx){
  ...
  ctx.memorize(()=>{
    ...
    function draw(message=level>=2?'Húzd a termékeket a kilenc számozott helyre. Kattintással is kiválaszthatod a terméket és a helyét.':'Töltsd meg a kilenchelyes kosarat. A termékek sorrendje nem számít.'){
      const used=new Set(chosen.filter(Boolean)),options=h('div',{className:'choice-grid association-shop-options'},...round.options.map(item=>h('button',{type:'button',className:`memory-tile object-tile${selected===item.id?' is-selected':''}`,disabled:used.has(item.id),draggable:true,onClick:()=>{selected=item.id;draw();},onDragstart:event=>{dragged=item.id;event.dataTransfer?.setData('text/plain',item.id);}},h('span',{className:'object-emoji','aria-hidden':'true'},item.emoji),h('strong',{},item.name))));
      const slots=h('div',{className:'association-cart','aria-label':'Kilenc kosárhely'},...chosen.map((id,index)=>{const item=round.options.find(option=>option.id===id);return h('button',{type:'button',className:`association-cart-slot${item?' is-filled':''}${selected===id?' is-selected':''}`,'aria-label':`${index+1}. kosárhely${item?`: ${item.name}`:''}`,draggable:!!item,onClick:()=>{if(selected&&selected!==id)place(selected,index);else if(id){selected=selected===id?null:id;draw();}},onDragstart:event=>{if(item){dragged=item.id;event.dataTransfer?.setData('text/plain',item.id);}},onDragover:event=>{if(dragged)event.preventDefault();},onDrop:event=>{event.preventDefault();if(dragged)place(dragged,index);}},h('span',{className:'association-slot-number'},`${index+1}.`),item?h('span',{},item.emoji,item.name):h('span',{},'üres'));}));
