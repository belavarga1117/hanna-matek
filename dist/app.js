...
import {createHannaHub} from './hanna/workspace.js';
...
function renderSetup(game,settings){
  ...
  if(game.id==='hanna-method'){
    ...
    let value;try{value=normalizeGameSettings(game.id,Object.keys(raw).length?raw:settings);}catch{value=normalizeGameSettings(game.id,{});}
    const component=createHannaSettings({h,value,school});active=component;settingsDraft={gameId:game.id,settings:value};
    shell(h('div',{className:'hanna-setup'},h('a',{href:'#/hanna-modszer',className:'back-link'},'← Hanna Módszer'),h('h1',{},'Válaszd ki a mai technikát'),component.element,h('button',{className:'primary-button',onClick:()=>{try{startGame(game,component.getValue());}catch(error){toast(error.message);}}},'Gyakorlat indítása')),'hanna');return;
  }
...
function renderResult(game,settings,result,saved){
  if(game.id==='hanna-method'){
    shell(h('section',{className:'result-panel hanna-complete'},h('span',{className:'eyebrow'},'HANNA MÓDSZER'),h('h1',{},'A mai kör elkészült.'),renderHannaResult(h,result),h('p',{className:saved||school?.user?.role==='teacher'?'saved-note':'input-error'},school?.user?.role==='teacher'?'Tanári előnézet. Tanulói eredményt nem rögzít.':saved?'✓ A válaszok és az eredmény a fiókodba mentve.':'Az eredmény most nem menthető.'),h('div',{className:'result-actions'},activeAssignment?h('a',{className:'primary-button',href:'#/feladataim'},'Vissza a feladataimhoz'):h('button',{className:'primary-button',onClick:()=>startGame(game,settings.activity==='review'?{...settings,reviewIds:[],reviewSnapshot:[]}:settings)},settings.activity==='review'?'Következő esedékes kör':'Új kör'),h('a',{className:'secondary-button',href:'#/hanna-modszer?view=daily'},'Napi terv folytatása'),h('a',{className:'secondary-button',href:'#/hanna-modszer'},'Hanna Módszer'))),'hanna');announce('A Hanna Módszer kör befejeződött.');return;
  }
...
function renderRoute(){if(!backendReady)return;stop();document.querySelectorAll('dialog').forEach(d=>d.remove());const rawPath=(location.hash||'#/').replace(/^#/,'').split('?')[0].replace(/\/$/,'')||'/';if(rawPath==='/hanna-modszer'){if(school?.supported&&!school.user){location.hash='#/fiok';return;}const component=createHannaHub({h,school,initialView:new URLSearchParams(location.hash.split('?')[1]||'').get('view')||'discover',onStart:settings=>startGame(games.find(game=>game.id==='hanna-method'),settings)});active=component;shell(h('div',{},component.element,renderHannaMethodSources(h)),'hanna');return;}if(rawPath==='/memoriaprobak'){const component=renderCognitiveHub({h,teacher:school?.user?.role==='teacher'});active=component;shell(component.element,'cognitive');return;}if(rawPath==='/memoriaprofil'){if(school?.supported&&!school.user){location.hash='#/fiok';return;}const component=createSchoolCognitiveProfile({h,school});active=component;shell(component.element,'cognitive');return;}if(school?.renderRoute())return;if(school?.supported&&!school.user){location.hash='#/fiok';return;}const pending=pendingEntries();if(pending.length){renderPending(pending[0]);return;}const route=parseRoute(location.hash,ids);if(route.page==='history')renderHistory();else if(route.page==='game')renderSetup(games.find(g=>g.id===route.id),route.settings);else renderHome();}
