...
function renderRoute(){if(!backendReady)return;stop();document.querySelectorAll('dialog').forEach(d=>d.remove());const rawPath=(location.hash||'#/').replace(/^#/,'').split('?')[0].replace(/\/$/,'')||'/';if(rawPath==='/memoriaprobak'){const component=renderCognitiveHub({h,teacher:school?.user?.role==='teacher'});active=component;shell(component.element,'cognitive');return;}if(rawPath==='/memoriaprofil'){if(school?.supported&&!school.user){location.hash='#/fiok';return;}const component=createSchoolCognitiveProfile({h,school});active=component;shell(component.element,'cognitive');return;}if(school?.renderRoute())return;if(school?.supported&&!school.user){location.hash='#/fiok';return;}const pending=pendingEntries();if(pending.length){renderPending(pending[0]);return;}const route=parseRoute(location.hash,ids);if(route.page==='history')renderHistory();else if(route.page==='game')renderSetup(games.find(g=>g.id===route.id),route.settings);else renderHome();}
