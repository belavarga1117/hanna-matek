...
async function startGame(game,rawSettings){
  ...
  const phaseTitle=h('h2',{tabIndex:-1},'Egy pillanat…'),phaseText=h('p',{className:'muted'}),root=h('div',{className:'game-stage'}),timerText=h('span',{className:'timer-number'},''),timerFill=h('div',{className:'timer-fill'}),timerWrap=h('div',{className:'timer-wrap',hidden:true},h('div',{className:'timer-label'},h('span',{},'Megjegyzés'),timerText),h('div',{className:'timer-track'},timerFill));
  ...
  try{
    ...
    const ctx={root,settings,rand:Math.random,h,phase(title,subtitle){if(disposed)return;phaseTitle.textContent=title;phaseText.textContent=subtitle||'';announce(title);},delay(fn,ms){const t=setTimeout(()=>{timers.delete(t);if(!disposed)fn();},ms);timers.add(t);return t;},memorize(callback){
      if(disposed)return;countdown?.cancel();timerWrap.hidden=false;ready.hidden=false;pause.hidden=false;
      countdown=createCountdown(settings.seconds,{onTick:left=>{timerText.textContent=`${Math.ceil(left)} mp`;timerFill.style.width=`${Math.max(0,left/settings.seconds*100)}%`;},onDone:()=>{if(disposed)return;countdown=null;timerWrap.hidden=true;ready.hidden=true;pause.hidden=true;overlay.hidden=true;root.inert=false;callback();phaseTitle.focus({preventScroll:true});}});
