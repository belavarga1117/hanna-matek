...
async function startGame(game,rawSettings,assignment=null){
  ...
  shell(h('div',{className:'play-page'},h('div',{className:'play-top'},h('button',{className:'back-link',onClick:()=>{stop();renderSetup(game,settings);}},'← Beállítások'),h('span',{className:`game-category ${game.color}`},game.title),pause),h('section',{className:'play-panel'},rulesVersion===1?h('p',{className:'muted'},'Korábban indított vagy kiosztott gyakorlat, az eredeti szabályaival.'):null,h('div',{className:'phase-heading'},phaseTitle,phaseText),timerWrap,root,h('div',{className:'ready-row'},ready),overlay),h('div',{className:'play-caption'},h('span',{},difficultyNames[settings.difficulty]),h('span',{},`${settings.seconds} mp ${game.id==='path'?'lejátszás':'megjegyzés'}`),h('span',{},'Időkorlát nélküli válaszadás'))),'game');
  try{
    const moduleKey=`${rulesVersion}:${game.module}`;
    const library=modules[moduleKey]||(modules[moduleKey]=await (rulesVersion===1?legacyModulesByName:modulesByName)[game.module]());
    if(token!==generation||disposed)return;
    if(!library[game.id]?.mount)throw new Error('A gyakorlat nem érhető el.');
    const ctx={root,settings,rand:seededRandom(localSeed),h,phase(title,subtitle){if(disposed)return;phaseTitle.textContent=title;phaseText.textContent=subtitle||'';announce(title);},delay(fn,ms){const t=setTimeout(()=>{timers.delete(t);if(!disposed)fn();},ms);timers.add(t);return t;},memorize(callback,{onProgress,allowSkip=true}={}){
    ...
    },done(result,answer){if(disposed||token!==generation)return;const normalized=normalizeResult(scoreAttempt(game.id,settings,localSeed,answer,rulesVersion));const entry={...normalized,gameId:game.id,at:new Date().toISOString(),settings,duration:Math.round((performance.now()-started)/1000)};dispose();active=null;
