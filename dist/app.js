...
import {seededRandom, normalizeGameSettings, GAME_RULES, COMMON_GAME_SETTINGS} from './game-engine.js';
...
function renderSetup(game,settings){
  ...
  const difficultyOptions=game.id==='grid'?[['easy','4 × 4 mező'],['normal','5 × 5 mező']]:game.id==='picture'?[['easy','4 hasonló kép'],['hard','6 hasonló kép']]:Object.entries(difficultyNames);
  ...
  const symbolSet=h('select',{id:'setting-symbols'},[['objects','Tárgyképek'],['abstract','Absztrakt jelek']].map(([v,label])=>h('option',{value:v,selected:v===s.symbolSet},label)));
  ...
  const difficultyField=['grid','prices','picture'].includes(game.id)?field('Nehézség','setting-difficulty',difficulty):null;
  const themeField=game.id==='stations'?field('Szókészlet','setting-theme',theme):null;
  function updateRule(){const value=Number(level.value),l=levels.find(v=>v.value===value)||levels[0];rule.textContent=l.description||game.description;steps.replaceChildren(...(l.steps||game.steps).map((text,i)=>h('li',{},h('span',{},String(i+1)),text)));if(countField)countField.hidden=game.id==='stations'&&value===2;if(themeField)themeField.hidden=value!==1;if(difficultyField)difficultyField.hidden=game.id==='picture'&&value===2;}
...
async function startGame(game,rawSettings,assignment=null){
  ...
  if(school?.supported&&!teacherPreview){
    ...
    try{const response=await school.api('/api/attempts',{method:'POST',body:{gameId:game.id,settings,...(assignment?.assignmentStepId?{assignmentStepId:assignment.assignmentStepId}:{})}});if(token!==generation||school.user?.id!==userId)return;attempt=response.attempt;settings=normalizeGameSettings(game.id,attempt.settings);}
  ...
  try{
    ...
    const ctx={root,settings,rand:attempt?seededRandom(attempt.seed):Math.random,h,phase(title,subtitle){if(disposed)return;phaseTitle.textContent=title;phaseText.textContent=subtitle||'';announce(title);},delay(fn,ms){const t=setTimeout(()=>{timers.delete(t);if(!disposed)fn();},ms);timers.add(t);return t;},memorize(callback,{onProgress,allowSkip=true}={}){
    ...
    },done(result,answer){if(disposed||token!==generation)return;const normalized=normalizeResult(result);const entry={...normalized,gameId:game.id,at:new Date().toISOString(),settings,duration:Math.round((performance.now()-started)/1000)};dispose();active=null;
...
async function loadAccountHistory(){if(!school.supported)return;if(!school.user||school.user.role!=='student'){history=[];return;}const userId=school.user.id;try{const data=await school.api('/api/results');if(school.user?.id===userId)history=[...(data.results||[])].sort((a,b)=>Date.parse(a.at)-Date.parse(b.at));}catch{history=[];}}
...
if(document.modelContext?.registerTool){
  ...
  register({name:'configure_memory_game',title:'Gyakorlat beállítása',description:'Open the visible setup page for a memory game; does not start a round or save a result.',inputSchema:{type:'object',properties:{gameId:{type:'string',enum:ids},count:{type:'integer',minimum:3,maximum:8},seconds:{type:'integer',minimum:3,maximum:30}},required:['gameId'],additionalProperties:false},execute:input=>{if(!input||!ids.includes(input.gameId)||Object.keys(input).some(k=>!['gameId','count','seconds'].includes(k))||['count','seconds'].some(k=>input[k]!==undefined&&(!Number.isInteger(input[k])||input[k]<(k==='count'?3:3)||input[k]>(k==='count'?8:30))))throw new Error('Invalid game or settings');const game=games.find(g=>g.id===input.gameId),settings=normalizeGameSettings(game.id,input);if(school?.supported&&!school.user)throw new Error('Bejelentkezés szükséges');stop();window.history.replaceState(null,'',linkFor(game,settings));renderSetup(game,settings);return {gameId:game.id,settings:settingsDraft.settings,page:'setup'};}});
