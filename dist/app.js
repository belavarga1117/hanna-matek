...
const modulesByName={core:()=>import('./games/core-games.js').then(m=>m.coreGames),association:()=>import('./games/association-games.js').then(m=>m.associationGames),advanced:()=>import('./games/advanced-games.js').then(m=>m.advancedGames),nback:()=>import('./nback/ui.js').then(m=>m.nbackGames)};
...
function settingsLabel(game,settings){return game.id==='nback'?describeNbackSettings(settings):difficultyNames[settings.difficulty];}
...
function mini(game,large=false){
  ...
  const symbols={digits:['2','8','4'],grid:['▪','▪','▪','▪','▪','▪','▪','▪','▪'],path:['1','·','3','·','2','·'],missing:['🍋','?','🔑'],stations:['A','—','B'],prices:['🍐','490'],shopping:['🍎','🥕','🥖'],picture:['●','▲','◆','■'],code:['3','=','🌵'],nback:['N','←','N']}[game.icon];
...
function renderHome(){
  ...
  const categoryNames=['Mind',...new Set(games.map(game=>game.category))];
  ...
  shell(h('div',{},h('section',{className:'intro'},h('div',{},h('span',{className:'eyebrow'},'EGY KIS GYAKORLÁS, A SAJÁT TEMPÓDBAN'),h('h1',{},'Ma mire ',h('em',{},'figyelsz?')),h('p',{},'Válassz egy játékot. Állítsd magadra. Kezdődhet.')),h('div',{className:'intro-badge'},h('strong',{},String(games.length)),h('span',{},'memória-',h('br'),'gyakorlat'))),h('section',{className:'start-strip'},h('div',{className:'start-strip-label'},h('span',{className:'tiny-star','aria-hidden':'true'},'✳'),h('div',{},h('span',{className:'eyebrow'},last?'FOLYTASD EGY ÚJ KÖRREL':'EGY JÓ KEZDÉS'),h('h2',{},featured.title))),h('p',{},last?'A legutóbbi beállításaiddal, friss feladattal.':'Jegyezz meg egy rövid számsort. Indulj öt számjeggyel.'),h('a',{className:'primary-button light-button',href:linkFor(featured,last?.settings||defaultSettings(featured))},'Kipróbálom',h('span',{'aria-hidden':'true'},'↗'))),h('div',{className:'catalog-top'},h('div',{className:'filter-list','aria-label':'Játékkategóriák'},filters),countLabel),grid,h('section',{className:'practice-note'},h('span',{className:'note-icon','aria-hidden':'true'},'◎'),h('div',{},h('h2',{},stats.rounds?'Már van mire visszanézned.':'Találd meg a saját ritmusod.'),h('p',{},stats.rounds?`${stats.rounds} befejezett kör, ${stats.games} kipróbált játék. Az eredményeid segítenek a következő szint kiválasztásában.`:'Kezdd több megjegyzési idővel. Ha már könnyen megy, növeld az elemszámot vagy csökkentsd az időt.')),stats.rounds?h('a',{href:'#/eredmenyek',className:'text-link'},'Eredményeim →'):null)),'home');
}
...
async function startGame(game,rawSettings,assignment=null){
  ...
  const caption=game.id==='nback'?[h('span',{},`N=${settings.n}`),h('span',{},`${settings.trialCount} pontozott inger`),h('span',{},settings.selfPaced?'Saját tempó':`${settings.intervalMs/1000} mp / inger`)]:[h('span',{},difficultyNames[settings.difficulty]),h('span',{},`${settings.seconds} mp ${game.id==='path'?'lejátszás':'megjegyzés'}`),h('span',{},'Időkorlát nélküli válaszadás')];
  shell(h('div',{className:`play-page ${game.id==='nback'?'nback-host-play':''}`},h('div',{className:'play-top'},h('button',{className:'back-link',onClick:()=>{stop();renderSetup(game,settings);}},'← Beállítások'),h('span',{className:`game-category ${game.color}`},game.title),game.id==='nback'?null:pause),h('section',{className:'play-panel'},rulesVersion===1?h('p',{className:'muted'},'Korábban indított vagy kiosztott gyakorlat, az eredeti szabályaival.'):null,h('div',{className:'phase-heading'},phaseTitle,phaseText),game.id==='nback'?null:timerWrap,root,game.id==='nback'?null:h('div',{className:'ready-row'},ready),game.id==='nback'?null:overlay),h('div',{className:'play-caption'},caption)),'game');
  try{
    ...
    },done(result,answer){if(disposed||token!==generation)return;const normalized=normalizeResult({...scoreAttempt(game.id,settings,localSeed,answer,rulesVersion),gameId:game.id});const entry={...normalized,gameId:game.id,at:new Date().toISOString(),settings,duration:Math.round((performance.now()-started)/1000)};dispose();active=null;
...
async function loadAccountHistory(){if(!school.supported)return;if(!school.user||school.user.role!=='student'){history=[];return;}const userId=school.user.id;try{const data=await school.api('/api/results');if(school.user?.id===userId)history=[...(data.results||[])].map(result=>({...result,...normalizeResult(result),settings:normalizeSettingsForVersion(result.gameId,result.settings,result.rulesVersion)})).sort((a,b)=>Date.parse(a.at)-Date.parse(b.at));}catch{history=[];}}
