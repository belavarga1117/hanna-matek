...
import {createCognitiveProfile} from './cognitive/profile.js';
...
function mini(game,large=false){
  ...
  const symbols={digits:['2','8','4'],grid:['▪','▪','▪','▪','▪','▪','▪','▪','▪'],path:['1','·','3','·','2','·'],missing:['🍋','?','🔑'],stations:['A','—','B'],prices:['🍐','490'],shopping:['🍎','🥕','🥖'],picture:['●','▲','◆','■'],code:['3','=','🌵'],nback:['N','←','N'],'spatial-span':['1','⌁','3'],'digit-span':['◌','7','2'],'picture-place':['🍎','▦','🧸'],'complex-span':['◇','+','⌁'],recognition:['◒','?','◐'],'attention-nogo':['●','★','▲'],'active-recall':['✎','?','↻']}[game.icon];
...
async function startGame(game,rawSettings,assignment=null){
  ...
  shell(h('div',{className:`play-page ${game.id==='nback'?'nback-host-play':''} ${cognitive?'cognitive-host-play':''}`},h('div',{className:'play-top'},h('button',{className:'back-link',onClick:()=>{stop();renderSetup(game,settings);}},'← Beállítások'),h('span',{className:`game-category ${game.color}`},game.title),game.id==='nback'||cognitive?null:pause),h('section',{className:'play-panel'},rulesVersion===1?h('p',{className:'muted'},'Korábban indított vagy kiosztott gyakorlat, az eredeti szabályaival.'):null,h('div',{className:'phase-heading'},phaseTitle,phaseText),game.id==='nback'||cognitive?null:timerWrap,root,game.id==='nback'||cognitive?null:h('div',{className:'ready-row'},ready),game.id==='nback'||cognitive?null:overlay),h('div',{className:'play-caption'},caption)),'game');
  try{
...
function renderRoute(){if(!backendReady)return;stop();document.querySelectorAll('dialog').forEach(d=>d.remove());const rawPath=(location.hash||'#/').replace(/^#/,'').split('?')[0].replace(/\/$/,'')||'/';if(rawPath==='/memoriaprobak'){const component=renderCognitiveHub({h});shell(component.element,'cognitive');return;}if(rawPath==='/memoriaprofil'){if(school?.supported&&!school.user){location.hash='#/fiok';return;}const component=createCognitiveProfile({h,loadResults:()=>school?.supported?school.api('/api/results'):Promise.resolve({results:[]})});shell(component.element,'cognitive');return;}if(school?.renderRoute())return;if(school?.supported&&!school.user){location.hash='#/fiok';return;}const pending=pendingEntries();if(pending.length){renderPending(pending[0]);return;}const route=parseRoute(location.hash,ids);if(route.page==='history')renderHistory();else if(route.page==='game')renderSetup(games.find(g=>g.id===route.id),route.settings);else renderHome();}
...
async function loadAccountHistory(){if(!school.supported)return;if(!school.user||school.user.role!=='student'){history=[];return;}const userId=school.user.id;try{const data=await school.api('/api/results');if(school.user?.id===userId)history=[...(data.results||[])].map(result=>({...result,...(isCognitiveGameId(result.gameId)?result:normalizeResult(result)),settings:normalizeSettingsForVersion(result.gameId,result.settings,result.rulesVersion)})).sort((a,b)=>Date.parse(a.at)-Date.parse(b.at));}catch{history=[];}}
